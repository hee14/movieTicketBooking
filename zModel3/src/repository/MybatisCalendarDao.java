...
public class MybatisCalendarDao extends AbstractRepository{
	...
	private final String namespace = "mybatis.CalendarMapper";
	private static MybatisCalendarDao instance = new MybatisCalendarDao();
	private MybatisCalendarDao(){}
	
	public static MybatisCalendarDao getInstance(){
		return instance;
	}
	
	public void insertEvent(Calendar event){
		SqlSession sqlSession = getSqlSessionFactory().openSession();
		String statement = null;
		//Map map = new HashMap<>();
		
		int cal_num = event.getCal_num();
		String mem_id = event.getMem_id();
		String cal_title = event.getCal_title();
		...
		try {
			...
			event.setCal_num(cal_num);
			event.setMem_id(mem_id);
			event.setCal_title(cal_title);
			
			statement = namespace + ".insert";
			sqlSession.insert(statement, event);
			sqlSession.commit();
		}finally {
			sqlSession.close();
		}
